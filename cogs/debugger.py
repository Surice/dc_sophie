...
class Debugger(commands.Cog):
    ...
    async def debug(self, ctx, *, option: str = None):
        ...
        try:
            if embed_perms(ctx.message):
                em = discord.Embed(color=0xad2929, title='Debug Infos')
                system = ''
    ...
    async def list(self, ctx, txt: str = None):
        ...
        try:
            ...
            if len(filelist) == 0:
                return await ctx.send('No saved cmd/scripts.')
            filelist.sort()
            msg = ''
            ...
            await ctx.send('List of saved cmd/scripts. Page ``%s of %s`` ```%s```' % (numb, pages, msg))
        except Exception as e:
    ...
    async def loadall(self, ctx):
        ...
        if not errors:
            await ctx.send("All core modules loaded")
        else:
